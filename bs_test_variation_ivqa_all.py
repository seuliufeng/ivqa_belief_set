...
def ivqa_decoding_beam_search(checkpoint_path=None):
    ...
    for i in range(num):
        ...
        # answers, vqa_scores = vqa_model.inference(image_id, sampled)
        # pdb.set_trace()
        # conf_inds = (-vqa_scores).argsort()[:20]
        # is_valid = [0, 0]
        _this_batch_size = len(sampled)
        vqa_scores = np.ones(shape=(_this_batch_size,), dtype=np.float32)
        is_valid = np.ones_like(vqa_scores, dtype=np.bool)
