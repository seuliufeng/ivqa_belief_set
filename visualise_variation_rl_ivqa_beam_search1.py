...
from var_ivqa_rewards import AttentionVQARewards
...
def extract_gt(capt, capt_len, use_array=True):
    gt = []
    for c, c_len in zip(capt, capt_len):
        ...
        if use_array:
            ...
            gt.append([START_TOKEN] + tmp + [END_TOKEN])
    return gt
...
def ivqa_decoding_beam_search(checkpoint_path=None, subset='kpval'):
    ...
    create_fn = create_reader('VAQ-EpochAtt', phase='test')
    writer = ExperimentWriter('latex/examples_var')
    ...
    env = AttentionVQARewards()

    if checkpoint_path is None:
        # ckpt_dir = FLAGS.checkpoint_dir % (FLAGS.version, FLAGS.model_type)
        ckpt_dir = 'model/v1_var_att_lowthresh_cache_restval_VAQ-VarRL'
        ckpt_dir = 'model/v1_var_ivqa_restval_VAQ-Var/'
    ...
    for i in range(num_batches):
        ...
        res5c, image, quest, quest_len, top_ans, ans, ans_len, quest_ids, image_ids = outputs
        import pdb

        # inference
        scores, pathes = model.greedy_inference([image, ans, ans_len], sess)
        scores, pathes = post_process_prediction(scores, pathes)
        pathes, pathes_len = put_to_array(pathes)
        scores, pathes = find_unique_rows(scores, pathes)
        scores, pathes = post_process_prediction(scores, pathes[:, 1:])

        gt_path = extract_gt(quest, quest_len, use_array=False)
        question = to_sentence.index_to_question(gt_path[0])
        print('%d/%d: %s' % (i, num_batches, question))

        vqa_inputs = [image, res5c, ans, ans_len, top_ans]
        env_pathes = [gt_path + pathes]
        pdb.set_trace()
        # pdb.set_trace()
        scores, _ = env.get_reward(env_pathes, vqa_inputs)
        # pdb.set_trace()
        ...
        title = 'Q:%s A: %s (%0.2f)' % (question, answer, scores[0])
        questions = []

        scores = scores[1:]
        for path, sc in zip(pathes, scores):
            sentence = to_sentence.index_to_question(path)
            questions.append('%s (%0.2f)' % (sentence, sc))
            print(sentence)
        # plt.show()
        writer.add_result(image_ids[0], quest_ids[0], im_path, title, questions)
