...
class VQABelief(object):
    ...
    def should_terminate(self):
        status = np.array(self.memory.values()) > 0.9
        valid_size = status.sum()
        do_term = (self.iter > 200 or valid_size >= self.n_keep)
        return do_term
    ...
    def _sort_memory(self):
        sorted_mem = sorted(self.memory.items(), key=operator.itemgetter(1), reverse=True)
        # pdb.set_trace()
        self.sorted_memory = sorted_mem

    def show_belief(self, env, quest_id):
        # os.system('clear')
        self._sort_memory()
        valid_len = min(len(self.sorted_memory), self.n_keep)
        print('Iter %d: Find %d unique questions for question %d' %
              (self.iter, valid_len, quest_id))
        exampels = []
        self.sampled = []
        for p, v in self.sorted_memory[:valid_len]:
            # v = self.memory[p]
            tokens = [int(t) for t in p.split(' ')]
            sent = env.to_sentence.index_to_question(tokens)
            self.sampled.append(tokens)
            p_str = '%s (%0.2f)' % (sent, v)
            exampels.append(p_str)
            print(p_str)
    ...
    def vertify_vqa(self, env, inputs):
        if self.should_terminate():
            self.show_belief(env, 0)
            scores, _ = env.vqa_reward.get_reward([self.sampled], inputs)
            print('Verified scores:')
            print(scores)
