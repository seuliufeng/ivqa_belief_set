...
def _process_dataset(subset, images, encoder):
    meta_filename = os.path.join(FLAGS.output_dir, 'vqa_std_mscoco_%s.meta' % subset)
    data_filename = os.path.join(FLAGS.output_dir, 'vqa_std_mscoco_%s.data' % subset)
...
def process_testset(subset):
    test = _load_and_process_metadata(subset)
    tf.logging.info('got %d testing images of %s' % (len(test), subset.upper()))
    ...
    _process_dataset(subset, test, encoder)


def main(_):
    process_testset('test-dev')
