...
class AttentionDataPrefetcher(Process):
    ...
    def _load_data(self):
        ...
        self._quest_ids = np.array(d['quest_id'], dtype=np.int32)
    ...
    def _get_sample_index(self):
        ...
        if self._pointer >= self._valid_count:
            self._shuffle_data()
        return index, quest_ids

    def pop_batch(self):
        # index = np.random.choice(self._valid_ids,
        #                          size=self._batch_size,
        #                          replace=False)
        # quest_ids = self._quest_ids[index]
        index, quest_ids = self._get_sample_index()
        outputs = [quest_ids]
        if self._output_im:
