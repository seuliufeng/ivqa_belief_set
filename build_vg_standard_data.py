...
tf.flags.DEFINE_string("output_dir", "data2/", "Output data directory.")
...
def _process_dataset(subset, images, encoder):
    meta_filename = os.path.join(FLAGS.output_dir, 'v7w_std_mscoco_%s.meta' % subset)
    data_filename = os.path.join(FLAGS.output_dir, 'v7w_std_mscoco_%s.data' % subset)
    ...
    tf.logging.info('In vocabulary percentage: %0.2f' % (100.-100.*num_oov/labels.size))
    # save data file
...
def main(_):
    ...
    if FLAGS.append_vg:
        ...
        vg_meta = _load_and_process_metadata_vg(black_list)
    ...
    if FLAGS.update_vocab:
        ...
        _create_top_answers_vocab(train, FLAGS.num_top_answers, subset='train')
        tf.logging.info('creating question vocabulary')
        _create_question_vocab(train, 'train')
        tf.logging.info('creating answer vocabulary')
        _create_answer_vocab(train, 'train')

    # # setup encoder
    encoder = create_sample_encoder('train')

    tf.logging.info('converting validation set')
    _process_dataset('val', val, encoder)
    tf.logging.info('converting testing set')
    _process_dataset('test', test, encoder)
    tf.logging.info('converting training set')
    _process_dataset('train', v7w_train, encoder)
    if FLAGS.append_vg:
        tf.logging.info('converting vg augmented training set')
        _process_dataset('vg_aug_train', train, encoder)
