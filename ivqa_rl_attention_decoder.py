...
def _build_training_decoder(glb_ctx, im, ans, inputs, length, targets, vocab_size,
                            num_cells, keep_prob, pad_token, rewards, advantage):
    ...
    # compute loss
    actor_loss = tf.div(tf.reduce_sum(losses * mask * advantage),
                        tf.reduce_sum(mask),
                        name='actor_loss')
...
def build_critic(glb_ctx, im, ans, quest, quest_len, vocab_size,
    ...
    # compute loss
    critic_loss = tf.div(tf.reduce_sum(tf.square(values - rewards) * mask * 0.5),
                         tf.reduce_sum(mask), name='critic_loss')
