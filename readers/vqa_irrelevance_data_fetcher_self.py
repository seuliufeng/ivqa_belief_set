...
class AttentionDataPrefetcher(Process):
    def __init__(self, output_queue, index_queue, proc_id,
        ...
        self._ez_ir_batch_size = int(self._batch_size * 0.5)
    ...
    def pop_batch(self):
        ...
        # concat
        im = np.concatenate([imr, im_ez], axis=0)
        quest = _concat_arrays(qr, qez)
        quest_len = np.concatenate([qlr, qlez])
        labels = np.concatenate([lbr, lbez])
