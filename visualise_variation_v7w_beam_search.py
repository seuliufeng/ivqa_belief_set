...
tf.flags.DEFINE_string("model_type", "VQA-VarDS",
...
tf.flags.DEFINE_string("test_version", "v1",
                       "Dataset version used for training, v1 for VQA 1.0, v2 "
                       "for VQA 2.0.")
tf.flags.DEFINE_string("trainset", "vg_aug_train",
                       "Dataset version used for training, v1 for VQA 1.0, v2 "
                       "for VQA 2.0.")
tf.flags.DEFINE_string("checkpoint_dir", "model/v7w_%s_%s",
...
def _get_vg_image_root(image_id):
    ...
    return os.path.join(im_root, pa_dir, fname)
...
def var_vqa_decoding_beam_search(checkpoint_path=None, subset='kpval'):
    ...
    to_sentence = SentenceGenerator(trainset='train',
