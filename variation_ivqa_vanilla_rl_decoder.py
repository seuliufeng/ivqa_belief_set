...
def build_actor(in_embed, quest, quest_len, advantages, vocab_size, keep_prob,
                pad_token, num_dec_cells):
    with tf.variable_scope('vaq'):
        inputs, targets, length = build_caption_inputs_and_targets(quest,
                                                                   quest_len)
        return _build_actor_network(in_embed, inputs, length, targets,
                                    advantages, vocab_size, num_dec_cells,
                                    keep_prob, pad_token)


# random sampler for sampling questions
def build_random_decoder(in_embed, vocab_size, num_dec_cells, keep_prob, epsion):
    with tf.variable_scope('vaq'):
        return _build_greedy_inference_decoder(in_embed,
                                               vocab_size,
                                               num_dec_cells,
                                               _START_TOKEN_ID,
                                               keep_prob,
...
def _build_actor_network(in_embed, inputs, length, targets, advantages,
    ...
    loss = tf.div(tf.reduce_sum(action_log_probs * advantages * mask), tf.reduce_sum(mask),
                  name='pg_loss')
