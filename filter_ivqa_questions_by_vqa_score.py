...
def process(K=1., use_global_thresh=False):
    ...
    # load VQA scores
    d = load_hdf5('data4/%sivqa_%s_qa_scores.data' % (_model_suffix, FLAGS.testset))
    cand_scores = d['ext_cand_scores']
    quest_ids = d['ext_quest_ids']

    # load QAs
    d = load_hdf5('data4/%sivqa_%s_question_answers.data' % (_model_suffix, FLAGS.testset))
    ...
    # save
    save_hdf5('data4/%sivqa_%s_question_answers_fltmax.data' % (_model_suffix, FLAGS.testset),
...
if __name__ == '__main__':
    process(K=0.3, use_global_thresh=True)
