...
class AttentionDataPrefetcher(Process):
    ...
    def pop_batch(self):
        ...
        q_w2v = self._slice_questions(index)
        a = self._slice_answers(index)
        outputs = [feats, q_w2v, a]
        return outputs
...
class AttentionFetcher(object):
    ...
    def get_test_batch(self):
        ...
        feats = self._slice_image(index)
        qw2v = self._slice_questions(index)
        a = self._slice_answers(index)
        quest_id = self._quest_ids[index]
        return feats, qw2v, a, quest_id, image_id
