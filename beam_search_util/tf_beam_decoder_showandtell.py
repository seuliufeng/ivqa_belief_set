...
def sparse_boolean_mask(tensor, mask):
    ...
    return tf.SparseTensor(
        ...
        shape=tf.cast([mask_shape[0], tf.reduce_max(mask_lens)], tf.int64)  # For 2D only
    )
...
def batch_gather(params, indices, validate_indices=None,
    ...
    flat_params = tf.reshape(params, concat_op([[batch_size_times_options_size], tf.shape(params)[2:]], axis=0))
...
class BeamSearchHelper(object):
    ...
    def beam_setup(self, time):
        ...
        cand_symbols = tf.fill([self.batch_size_times_beam_size, 0], tf.constant(self.stop_token, dtype=tf.int32))  # keep top k
