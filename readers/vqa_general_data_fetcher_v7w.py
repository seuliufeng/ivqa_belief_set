...
class AttentionDataPrefetcher(Process):
    ...
    def _load_data(self):
        meta_file = 'data2/v7w_std_mscoco_%s.meta' % self._subset
        data_file = 'data2/v7w_std_mscoco_%s.data' % self._subset
        d = load_json(meta_file)
        self._images = d['images']
        self._vqa_image_ids = np.array(d['image_ids'])
    ...
    def _load_global_image_feature(self):
        data_file = 'data2/v7w_res152_%s.h5' % self._subset
        d = load_hdf5(data_file)
    ...
    def _check_valid_answers(self):
        if False and self._output_qa and not self._output_ans_seq:
...
class AttentionTestDataFetcher(object):
    ...
    def _load_data(self):
        ...
        self._vqa_image_ids = np.array(d['image_ids'], dtype=np.int32)
        # load data file
        d = load_hdf5(data_file)
        self._quest = d['quest_arr'].astype(np.int32)
        self._quest_len = d['quest_len'].astype(np.int32)
