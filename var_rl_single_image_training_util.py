...
from rl_single_trainstep import reinforce_trainstep
from rl_single_trainstep import VQABelief
...
def feed_train(train_op, train_dir, log_every_n_steps,
    ...
    for itr in range(number_of_steps):
        ...
        quest_id = datum[-2][0]
        top_ans_id = datum[3][0]
        if top_ans_id == 2000:
            continue
        print('Hacking question %d...' % quest_id)
        t = time.time()
        while True:
        ...
        _VQA_Belief.show_belief(env, quest_id)
        _VQA_Belief.clear()
        # reset model
        init_fn(sess)
