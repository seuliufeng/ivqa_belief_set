...
tf.flags.DEFINE_string("checkpoint_dir", "/scratch/fl302/inverse_vqa/model/dbg_%s_kpvaq_%s_%s",
...
def ivqa_decoding_beam_search(checkpoint_path=None, subset='kpval'):
    ...
    # Get model
    model_fn = get_model_creation_fn('VAQ-2Att')
    ...
    with g.as_default():
        ...
        restorer = Restorer(g, warpper_scope='AgentVAQ')
        sess = tf.Session()
...
def main(_):
    ...
    ckpt_dir = FLAGS.checkpoint_dir % (FLAGS.version, 'VAQ-CA', FLAGS.loss_type)
    print(ckpt_dir)
