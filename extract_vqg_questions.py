...
tf.flags.DEFINE_string("test_version", "v1",
                       "Dataset version used for training, v1 for VQA 1.0, v2 "
                       "for VQA 2.0.")
tf.flags.DEFINE_string("checkpoint_dir", "model/%s_var_vqg_restval_%s",
...
def ivqa_decoding_beam_search(checkpoint_path=None, subset=FLAGS.subset):
    ...
    res_file = 'data4/%svqg_%s_questions.json' % (_model_suffix, FLAGS.subset)
    # Get model
    ...
    save_hdf5('data4/%svqg_%s_question_tokens.data' % (_model_suffix,
                                                        FLAGS.subset),
