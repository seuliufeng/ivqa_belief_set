...
from ops import concat_op, split_op, mlb
...
def build_decoder(fused, noise, ans, ans_len, vocab_size,
    ...
    with tf.variable_scope('var_vqa'):
        if phase == 'train' or phase == 'condition':
            ...
            inputs, targets, length = build_caption_inputs_and_targets(ans,
                                                                       ans_len)
...
def _build_training_decoder(in_embed, inputs, length, targets, vocab_size,
    ...
    logits_norm = 1.0 / tf.reshape(tf.cast(length, tf.float32), [-1, 1])  # normalise by length
