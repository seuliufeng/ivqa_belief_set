...
from rnn_ops import *
from ops import concat_op, select_op, unpack_op
...
def sparse_boolean_mask(tensor, mask):
    ...
    return tf.SparseTensor(
        tf.where(left_shifted_mask),
        tf.boolean_mask(tensor, mask),
        tf.cast([mask_shape[0], tf.reduce_max(mask_lens)], tf.int64)  # For 2D only
    )
...
def batch_gather(params, indices, validate_indices=None,
    ...
    flat_params = tf.reshape(params, concat_op([[batch_size_times_options_size], tf.shape(params)[2:]], 0))
...
class BeamFlattenWrapper(RNNCell):
    ...
    def merge_batch_beam(self, tensor):
        ...
        res = tf.reshape(tensor, concat_op([[-1], remaining_shape], 0))
    ...
    def unmerge_batch_beam(self, tensor):
        ...
        res = tf.reshape(tensor, concat_op([[-1, self.beam_size], remaining_shape], 0))
    ...
    def tile_along_beam(self, state):
        ...
        dynamic_tensor_shape = unpack_op(tf.shape(tensor))
...
class BeamReplicateWrapper(RNNCell):
    def __init__(self, cell, beam_size):
        self.cell = cell
        self.beam_size = beam_size
...
class BeamSearchHelper(object):
    ...
    def beam_setup(self, time):
        ...
        cand_logprobs.set_shape(tf.TensorShape((self.inferred_batch_size,)))
        beam_symbols.set_shape(tf.TensorShape((self.inferred_batch_size_times_beam_size, None)))
        beam_logprobs.set_shape(tf.TensorShape((self.inferred_batch_size_times_beam_size,)))
    ...
    def beam_loop(self, time, cell_output, cell_state, loop_state):
        ...
        logprobs_batched = tf.reshape(
            logprobs + tf.expand_dims(tf.reshape(past_beam_logprobs, [self.batch_size, self.beam_size]), 2),
        ...
        symbols_history = flat_batch_gather(past_beam_symbols, parent_refs, batch_size=self.batch_size,
                                            options_size=self.beam_size)
        beam_symbols = concat_op([symbols_history, tf.reshape(symbols, [-1, 1])], 1)
        ...
        done_symbols = flat_batch_gather(past_beam_symbols, done_parent_refs, batch_size=self.batch_size,
                                         options_size=self.beam_size)

        logprobs_done_max = tf.reduce_max(logprobs_done, 1)

        # Make sure the end token scores higher than all the top K partial captions
        update_cond = tf.logical_and(tf.greater(logprobs_done_max, past_cand_logprobs),  # larger than previous
                                     tf.greater(logprobs_done_max, min_beam_logprobs))  # in top K
        cand_symbols_unpadded = select_op(update_cond,
                                          done_symbols,  # current estimate
                                          past_cand_symbols)
        cand_logprobs = select_op(update_cond,
                                  logprobs_done_max,
                                  past_cand_logprobs)
        ...
        cand_symbols = concat_op([cand_symbols_unpadded, tf.fill([self.batch_size, 1], self.stop_token)], 1)
        ...
        if self.score_upper_bound is not None:
            elements_finished_bound = tf.reduce_max(tf.reshape(beam_logprobs, [-1, self.beam_size]), 1) < (
                cand_logprobs - self.score_upper_bound)
        ...
        for tensor in list(nest.flatten(next_input)) + list(nest.flatten(next_cell_state)):
            tensor.set_shape(
                tf.TensorShape((self.inferred_batch_size, self.beam_size)).concatenate(tensor.get_shape()[2:]))
        ...
        for tensor in [beam_symbols, beam_logprobs]:
            tensor.set_shape(
                tf.TensorShape((self.inferred_batch_size_times_beam_size,)).concatenate(tensor.get_shape()[1:]))
    ...
    def decode_dense(self):
        ...
        # return beam_symbols, beam_logprobs
