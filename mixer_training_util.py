...
def add_start_end_token(capt, capt_len):
    gt = extract_gt(capt, capt_len)
    arr, arr_len = put_to_array(gt)
    return arr, arr_len, gt
...
def feed_train(train_op, model, train_dir, log_every_n_steps,
    ...
    for itr in range(number_of_steps):
        ...
        # random sampling
        capt, capt_len, gts = add_start_end_token(capt, capt_len)
        rand_pathes, mask = model.mixer_sampling([im, attr,
                                                  ans_seq, ans_seq_len,
                                                  capt, capt_len], n_free_run_steps, sess)
        # print(to_sentence.index_to_question(rand_pathes[0]))
        # pdb.set_trace()
        xe_mask = put_to_array(mask)[0][:, 1:].astype(np.bool).flatten()  # remove start token
        ...
        inputs = [im, attr, rand_path, rand_path_len, ans_seq, ans_seq_len, rewards, advantage, xe_mask]

        total_loss, np_global_step = sess.run([train_op, global_step],
                                              feed_dict=feed_fn(inputs, n_free_run_steps))
