...
def _load_dataset():
    # load trainval
    subset = 'trainval'
    data_root = '/import/vision-ephemeral/fl302/code/VQA-tensorflow/'
    vqa_meta_file = data_root + 'data/vqa_std_mscoco_%s.meta' % subset
    vqa_meta_trainval = load_json(vqa_meta_file)
    vqa_data_file = data_root + 'data/vqa_std_mscoco_%s.data' % subset
    vqa_data_trainval = load_hdf5(vqa_data_file)

    attr_data_file = data_root + 'data/attribute_std_mscoco_%s.data' % subset
