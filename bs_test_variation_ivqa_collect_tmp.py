...
def ivqa_decoding_beam_search(ckpt_dir, method):
    ...
    # method = FLAGS.method
    if inf_type == 'rand':
        res_file = 'result/tmp_bs_RL2_cands_LM_%s.json' % method
    else:
        res_file = 'result/tmp_bs_RL2_cands_LM_%s_BEAM.json' % method
    ...
    # set answers of interest
    answers = ['yes', 'no', 'green', 'white', '1']
    answers_in_token = [2, 3, 12, 5, 6]
    ...
    for i in range(num):
        ...
        for ans, ans_t in zip(answers, answers_in_token):
            ans_tokens = np.array([ans_t], dtype=np.int32).reshape([1, 1])
            ans_len = np.array([1], dtype=np.int32)
...
def main(_):
    models = ['MLB-att', 'N2NMN', 'SL']
    ckpts = ['model/v1_var_rl_att2_restval_VAQ-VarRL2_r2',
             'model/v1_var_rl_n2nmn_VAQ-VarRL2',
             'model/v1_var_ivqa_restval_VAQ-Var']
    for model, ckpt_dir in zip(models, ckpts):
        # try:
        ivqa_decoding_beam_search(ckpt_dir=ckpt_dir, method=model)
        # except Exception:
        #     continue
