...
def _load_and_process_metadata_vqg(subset):
...
def _process_dataset(subset, images, encoder):
    meta_filename = os.path.join(FLAGS.output_dir, 'vqg_question_ids_vqavoc.meta')
...
def main(_):
    meta = load_vqg_meta()
    tf.logging.info('Collected %d QA pairs from Visual Genome Main dataset' % len(meta))
...
def read_csv(filename):
    import csv
    fname = os.path.join('../../data/VQG', filename)
    meta = []
    ...
    with open(fname, 'r') as fs:
        ...
        for row in reader:
            ...
            for question in questions:
                question = _tokenize_sentence(question)
                meta.append(ImageMetadata(None, None, None, question, None, None, None))
    return meta


def load_vqg_meta():
    fnames = os.listdir('../../data/VQG')
    meta = []
    for fname in fnames:
        meta += read_csv(fname)
    return meta
