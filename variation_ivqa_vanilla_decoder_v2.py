...
def build_decoder(im, conditions, quest, quest_len, vocab_size,
                  keep_prob, pad_token, num_dec_cells, phase='train'):
    # average pooling over image
    answer_embed, noise = conditions
    answer_reduct = slim.fully_connected(answer_embed, num_dec_cells,
    ...
    with tf.variable_scope('vaq'):
        if phase == 'train' or phase == 'condition':
            inputs, targets, length = build_caption_inputs_and_targets(quest,
                                                                       quest_len)
            return _build_training_decoder(in_embed, z_embed, inputs, length, targets,
            ...
            return _build_tf_beam_inference_decoder(in_embed, z_embed, vocab_size, num_dec_cells,
                                                   _START_TOKEN_ID)
        else:
...
# *****************  TRAINING GRAPH *******************************
def _build_training_decoder(in_embed, z_embed, inputs, length, targets, vocab_size,
    ...
    with tf.variable_scope('RNN') as sc:
        # feed z
        _, new_state = lstm(z_embed, init_state)
        tf.get_variable_scope().reuse_variables()
        outputs, states = tf.nn.dynamic_rnn(lstm, inputs, length,
...
# *****************  Beam Search GRAPH *******************************
def _build_tf_beam_inference_decoder(in_embed, z_embed, vocab_size, num_cells, start_token_id):
    vocab_size += 1
    # init state / image embedding
    init_h = slim.fully_connected(in_embed, num_cells, activation_fn=tf.nn.tanh,
                                  scope='init_h')
    ...
    with tf.variable_scope('RNN') as sc:
        _, new_state = lstm_cell(z_embed, init_state)
        tf.get_variable_scope().reuse_variables()
