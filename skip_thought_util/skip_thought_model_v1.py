...
class SkipThoughtGRUCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
            with vs.variable_scope("Gates"):  # Reset gate and update gate.
                ...
                r, u = array_ops.split(_linear([inputs, state], 2 * self._num_units,
                                               True, 1.0), 2, 1)
...
class SkipThoughtEncoder(object):
    def __init__(self, vocab_size, keep_prob=1.0,
        ...
        if self._use_default:
            ...
            self._gru_fn = tf.contrib.rnn.GRUCell
        self.build_model()
    ...
    def build_model(self):
        ...
        self._cells = tf.contrib.rnn.DropoutWrapper(cells, input_keep_prob=self._keep_prob,
                                                    output_keep_prob=self._keep_prob)

    def __call__(self, inputs, input_len, scope='SkipThought'):
