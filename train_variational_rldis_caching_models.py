...
import var_rl_cache_training_util as training_util
...
tf.flags.DEFINE_string("version", "v1",
                       ...
                       "for VQA 2.0.")
tf.flags.DEFINE_string("train_dir", "model/%s_var_ivqa_dis_cache_restval_%s",
...
def train():
    ...
    env = MixReward(dis_vqa_reward=True)
    env.diversity_reward.mode = 'winner_take_all'
    env.set_language_thresh(0.1)
    env.set_replay_buffer(insert_thresh=0.5,
                          sv_dir='vqa_replay_buffer/disc')  # if 0.5, already fooled others
