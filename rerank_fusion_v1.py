...
class RerankModel(object):
    ...
    def _build_gates(self):
        # cand_embed_reduct = slim.fully_connected(self.cand_embed, 300,
        #                                          activation_fn=tf.nn.tanh,
        #                                          scope='cand_reduct')
        # cand_embed_reduct = slim.dropout(cand_embed_reduct, self.keep_prob)
        # context states
        ctx_0 = tf.concat([self.im_embed, self.quest_embed], axis=1)
        # ctx = slim.fully_connected(ctx_0, 300, activation_fn=tf.nn.tanh, scope='context')
        # ctx = tf.nn.dropout(ctx, self.keep_prob)
        # importance key
        # att_key = slim.fully_connected(ctx, 300, activation_fn=tf.nn.tanh, scope='att_key')
        att_key = ctx_0
        att_key = tf.tile(tf.expand_dims(att_key, 1), [1, self.num_cands, 1])
        joint = tf.concat([att_key, self.cand_embed], axis=2)
        bias_feat = slim.fully_connected(joint, 4096, activation_fn=tf.nn.relu, scope='bias_feat')
        bias_feat = slim.dropout(bias_feat, keep_prob=self.keep_prob)
        match_bias = slim.fully_connected(bias_feat, 1, activation_fn=tf.nn.sigmoid, scope='bias')
        match_bias = tf.squeeze(match_bias, 2)
        self.match_mask = match_bias
        # key_strength = slim.fully_connected(ctx, 1, activation_fn=tf.nn.softplus,
        #                                     scope='att_key_strength')
        # att_key = tf.expand_dims(att_key, axis=2)
        # cand_logits = tf.matmul(self.cand_embed, att_key)
        # cand_logits = tf.squeeze(cand_logits, axis=2)
        # cand_logits = tf.squeeze(cand_logits, axis=2) * key_strength
        # cand_logits = tf.nn.softmax(cand_logits)
        # cand_mask = tf.nn.sigmoid(cand_logits)

        # compute temp
        # temp = slim.fully_connected(ctx, 1, activation_fn=tf.nn.softplus, scope='softmax_temp')
        # # temp = 1.0
        # self._norm_ivqa_scores = softmax_norm(self._ivqa_scores, temp) * self.match_mask
        # # self._norm_ivqa_scores = softmax_norm(self._ivqa_scores, temp)
        # # self._norm_ivqa_scores = match_bias
    ...
    def _build_loss(self):
        ...
        # mask = tf.tile(tf.expand_dims(mask, axis=1), [1, self.num_cands])
        # mask = tf.reshape(tf.cast(mask, tf.float32), [batch_size, self.num_cands])
        ...
        rank_main = ranking_loss(self.match_mask, gt_mask)
