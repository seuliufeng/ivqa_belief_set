...
tf.flags.DEFINE_string("annotation_dir", "/import/vision-ephemeral/fl302/data/VQA2.0",
                       "Directory containing all annotation files.")

tf.flags.DEFINE_string("word2vec_model", "../VQA-tensorflow/data/GoogleNews-vectors-negative300.bin",
...
ImageMetadata = namedtuple("ImageMetadata",
                           ["image_id", "filename", "question_id", "question", "answer", "token_answer",
                            "counter_example"])
...
def create_sample_encoder(trainset='trainval'):
    ...
    vocab_name = 'vqa_%s_answer' % trainset
    answer_vocab_file = FLAGS.word_counts_output_file % vocab_name
    encoder.register_encoder(AnswerEncoder('sequence', answer_vocab_file))
    encoder.finalize()
    return encoder
...
def _load_vocab(vocab_file):
    ...
    reverse_vocab = [line.strip().split()[0] for line in reverse_vocab]
...
def _list_tokens_to_array(data_tokens):
    num_images = len(data_tokens)
    data_len = [len(q) for q in data_tokens]
    max_len = max(data_len)
    data_arr = np.zeros([num_images, max_len], dtype=np.int32)
    for i, x in enumerate(data_arr):
        x[:data_len[i]] = data_tokens[i]
    data_len = np.array(data_len, dtype=np.int32)
    return data_arr, data_len


def _process_dataset(subset, images, encoder):
    meta_filename = os.path.join(FLAGS.output_dir, 'v2_vqa_std_mscoco_kp%s.meta' % subset)
    data_filename = os.path.join(FLAGS.output_dir, 'v2_vqa_std_mscoco_kp%s.data' % subset)
    ...
    save_hdf5(ans_filename, {'ans_arr': ans_arr,
                             'ans_len': ans_len})
    # save meta file
...
def split_validation_set(meta):
    train_image_ids = get_subset_image_ids('trainval')
    # make hashing table
    train_image_hash_table = {id: id for id in train_image_ids}
    train, val = [], []
    for datum in meta:
        image_id = datum.image_id
        if image_id in train_image_hash_table:
            train.append(datum)
        else:
            val.append(datum)
    return train, val
...
def get_meta_by_image_ids(meta, image_ids):
    image_id_hash_table = {id: id for id in image_ids}
    d_sub = []
    for datum in meta:
        image_id = datum.image_id
        if image_id in image_id_hash_table:
            d_sub.append(datum)
    return d_sub


def get_kpsplit_meta(meta, subset):
    image_ids = get_image_id(subset)
    return get_meta_by_image_ids(meta, image_ids)
...
def main(_):
    ...
    train = get_kpsplit_meta(meta, 'train')
    test = get_kpsplit_meta(meta, 'test')
    val = get_kpsplit_meta(meta, 'val')
    ...
    # Training set
    tf.logging.info('converting train set')
    _process_dataset('train', train, encoder)

    # Validation set
    tf.logging.info('converting val set')
    _process_dataset('val', val, encoder)

    # Test set
    tf.logging.info('converting test')
