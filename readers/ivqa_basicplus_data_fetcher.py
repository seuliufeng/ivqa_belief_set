...
class AttentionDataPrefetcher(Process):
    ...
    def _load_data(self):
        ...
        self._load_fused_attribute_data()
    ...
    def _load_fused_attribute_data(self):
        # load res5c feature
        data_file = 'data/res152_std_mscoco_%s.data' % self._subset
        d = load_hdf5(data_file)
        image_ids = d['image_ids']
        ...
        vqa_index2att_index = [image_id2att_index[image_id] for image_id in self._vqa_image_ids]
        self._vqa_index2att_index = np.array(vqa_index2att_index, dtype=np.int32)
        res5c_feats = d['features']
        # load attributes
        data_file = 'data/attribute_std_mscoco_%s.data' % self._subset
        d = load_hdf5(data_file)
        att_data = d['att_arr'].astype(np.float32)
        attr_image_ids = d['image_ids']
        attr_image_id2att_index = {image_id: i for i, image_id in enumerate(attr_image_ids)}
        # re-order attribute data
        attr_index = np.array([attr_image_id2att_index[image_id] for image_id in image_ids])
        reorder_attr = att_data[attr_index]

        self._attributes = np.concatenate([res5c_feats, reorder_attr], axis=1)
...
class AttentionTestDataFetcher(object):
    ...
    def _load_global_image_feature(self):
        subset = 'kptest' if self._subset == 'bs_test' else self._subset
