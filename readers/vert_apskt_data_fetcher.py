...
class AttentionDataPrefetcher(Process):
    ...
    def _load_answer_proposals(self):
        data_file = os.path.join(_DATA_ROOT,
                                 'data3/%svqa_ap_w2v_coding_%s.data' %
                                 (self._version_suffix, self._subset))
        d = load_hdf5(data_file)
        quest_ids = d['quest_ids']
        self._answer = d['cands_w2v'].astype(np.float32)
        self._labels = d['labels']
        assert (np.all(np.equal(quest_ids, self._quest_ids)))

        self.num_cands = self._answer.shape[-1] / 300
    ...
    def sample_answers(self, index):
        if self._sample_negative:
            ...
            answer_w2v = []
            for idx in index:
                _neg = self._neg_labels[idx]
                _pos_ids = self._pos_labels[idx]
                _neg_ids = np.random.choice(_neg, size=(2,),
                                            replace=False)
                _ids = [_pos_ids] + _neg_ids.tolist()
                _label = 0
                _w2v = self._answer[idx, _ids, :].reshape([1, -1])
                labels.append(_label)
                answer_w2v.append(_w2v)
    ...
    def pop_batch(self):
        ...
        outputs = [feats, q, q_len, answer_w2v, labels]
        return outputs
...
class TestReader(object):
    ...
    def _load_answer_proposals(self):
        ...
        quest_ids = d['quest_ids']
        self._answer = d['cands_w2v'].astype(np.float32)
        self._cand_scores = d['cands_scs'].astype(np.float32)
        self._labels = None
        assert (np.all(np.equal(quest_ids, self._quest_ids)))
        if 'is_oov' in d:
            self._is_oov = d['is_oov']

        cand_file = os.path.join(_DATA_ROOT,
                                 'data3/%svqa_ap_cands_%s.meta' %
                                 (self._version_suffix, self._subset))
        # load candiates
        self._cand_ans = load_json(cand_file)
        self.num_cands = len(self._cand_ans[0]['cands'])
    ...
    def get_test_batch(self):
        ...
        q, q_len = self._slice_questions(index)
        answer_w2v = self._slice_answers(index)
        cand_scs = self._cand_scores[index]
        cands = [self._cand_ans[idx] for idx in index]
        return feats, q, q_len, answer_w2v, cand_scs, cands, quest_id, image_id

    def get_in_vocab_state(self, quest_ids):
        assert (np.all(np.equal(quest_ids, self._quest_ids)))
        return np.logical_not(self._is_oov)
