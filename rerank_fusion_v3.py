...
def encode_states(embedding, scores, keep_prob, dim, scope):
    with tf.variable_scope(scope):
        ...
        state = slim.fully_connected(avg_embed, dim, activation_fn=tf.nn.tanh,
                                     scope='hidden')
...
def out_product(x, y):
    ...
    outprod = tf.matmul(x, y)
    return slim.flatten(outprod)


def inner_product_3d(x, y):
    x = tf.expand_dims(x, 2)
    y = tf.expand_dims(y, 3)
    inprod = tf.matmul(x, y)
    return tf.squeeze(inprod, 3)
...
class RerankModel(object):
    ...
    def _build_predictor(self):
        ...
        # submodel states
        dummy = tf.ones_like(self._vqa_scores)
        # dummy = self._vqa_scores
        vqa_states = encode_states(self.cand_embed, dummy,
                                   self.keep_prob, 64, 'state_vqa')
        # vqa_states = slim.dropout(vqa_states, keep_prob=self.keep_prob)
        ivqa_states = encode_states(self.cand_embed, self._ivqa_scores,
                                    self.keep_prob, 64, 'state_ivqa')
        states = out_product(vqa_states, ivqa_states)
        states = slim.dropout(states, self.keep_prob)
        state_embed = slim.fully_connected(states, 256, activation_fn=tf.nn.tanh,
                                           scope='key')
        state_embed = tf.expand_dims(state_embed, 2)

        # compute outputs
        cand_embed = slim.fully_connected(self.cand_embed, 256, activation_fn=tf.nn.tanh,
                                          scope='values')
        preds = tf.squeeze(tf.matmul(cand_embed, state_embed), axis=2)
        temp2 = slim.fully_connected(ctx, 1, activation_fn=tf.nn.softplus,
                                     scope='softmax_temp2')
        self.logits = temp2 * preds
