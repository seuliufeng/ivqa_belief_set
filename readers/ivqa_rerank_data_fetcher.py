...
class AttentionDataPrefetcher(Process):
    def __init__(self, queue, proc_id, batch_size=32,
        ...
        self._version = version
        self._data_root = '../iccv_vaq/' if version == 'v1' else ''
        # data buffers
        self._images = None
        # vqa
    ...
    def _load_data(self):
        meta_file = self._data_root + 'data/%svqa_std_mscoco_%s.meta' % (self._version_suffix, self._subset)
        data_file = self._data_root + 'data/%svqa_std_mscoco_%s.data' % (self._version_suffix, self._subset)
        # load meta
    ...
    def _check_valid_answers(self):
        if self._output_qa and not self._output_ans_seq and False:
            print('Filter question/answers')
            self._valid_ids = np.where(self._answer < self._num_top_ans)[0]
        else:
            print('Don''t filter question/answers')
    ...
    def pop_batch(self):
        ...
        outputs.append(self._quest_ids[index])
        return outputs
